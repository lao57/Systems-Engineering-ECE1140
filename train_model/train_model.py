...
class TrainModel:
    ...
    def update_train(self, world_time):
        ...
        # TODO: remove self.acceleration
        ebrake, sbrake_decel, cmd_power, modified_cabin_temp, open_doors, open_lights, announcement = \
            self.train_controller.iterate(self.acceleration, self.previous_acceleration, self.cmd_velocity,
        ...
        # if self.ebrake:
        #     self.acceleration = (
        #                 0 - self.sbrake_decel * service_brake_deceleration - self.ebrake * emergency_brake_deceleration - gravitational_acceleration - rolling_resistance)
        # else:
        ...
        self.acceleration = force / self.mass - (emergency_brake_deceleration * self.ebrake + self.sbrake_decel +
                                                 gravitational_acceleration)

        self.previous_velocity = self.velocity
        self.velocity = self.previous_velocity + (1 / 2) * (self.acceleration + self.previous_acceleration) * self.dt
        self.velocity = max(self.velocity, 0)

        # distance handling
        distance_over_interval = (1 / 2) * (
                self.velocity + self.previous_velocity) * self.dt  # one second times the average velocity
