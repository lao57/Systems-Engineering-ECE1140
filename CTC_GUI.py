...
class CTCGUI(QMainWindow, Ui_MainWindow):
    ...
    def manual_stop_selection(self):
        # Get the line from the UI element (assumed to be 'scheduleTrainLine').
        ...
        # Create an empty list to collect stop block numbers.
        stops = []
        # Get the list of possible stops based on the selected line.
        if line == "Green Line":
        ...
        # Insert a "Done" option to signal completion.
        possible_stops.insert(0, "Done")

        # Allow the user to select as many stops as desired.
        while True:
            ...
            if stop_item == "Done":
                break
            # Extract the integer block number.
            stop_block = int(stop_item.replace("Block ", ""))
            stops.append(stop_block)

        # Call the manual scheduling method using the collected stops.
