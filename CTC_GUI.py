...
class CTCGUI(QMainWindow, Ui_MainWindow):
    ...
    def manual_stop_selection(self):
        ...
        #lets user select a stop from the list.
        stop_item, ok = QInputDialog.getItem(self, "Select Stop", "Stop:", possible_stops, 0, False)
        if not ok:
            return

        # parse out the block number
        stop_block = int(stop_item.replace("Block ", ""))

        # call CTCOffice to schedule the manual train.
        self.ctc_office.schedule_manual_train(line, train_id, stop_block)

        # update GUI for new train
        self.update_all()

        QMessageBox.information(self, "Success",
                                f"Manually scheduled Train {train_id} on {line} to stop at Block {stop_block}.")
    ...
    def update_system_analysis(self):
        #get throughput if any active train has reached block 58, set throughput to 1
        throughput = 0
        for train in self.ctc_office.active_trains:
            if train.current_block and train.current_block.block_number == 58:
                throughput = 1
                break

        # update table
        item = QTableWidgetItem(str(throughput))
        item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
        self.tableSystemAnalysis.setItem(0, 2, item)
