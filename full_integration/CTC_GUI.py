...
from ctc_office import CTC, ScheduleLoader, load_track_layout, CTCOffice, STATION_BLOCKS_GREEN, STATION_BLOCKS_RED


class CTCGUI(QMainWindow, Ui_MainWindow):

    def __init__(self, ctc=None, ctc_office=None, track_layout=None, schedule_loader=None, track_controller=None, red_track_controller=None):
        super().__init__()
        self.setupUi(self)
        ...
        if self.track_controller:
            self.ctc.connect_track_controller(self.track_controller)

        if self.red_track_controller:
            self.ctc.connect_red_track_controller(self.red_track_controller)
    ...
    def load_schedule(self):
        ...
        if path:
            try:
                ...
                # Only schedule the trains for the line selected in the scheduleTrainLine QComboBox.
                line_selected = self.scheduleTrainLine.currentText().strip().title()
                if line_selected in schedules:
                    for idx in range(len(schedules[line_selected])):
                        self.ctc_office.schedule_train(line_selected, idx)
    ...
    def update_block_combobox(self):
        line = self.maintLine.currentText().strip().title()
        # Get a list of block numbers for the selected line.
        blocks = [str(b['block_number']) for b in self.track_layout.get(line, [])]
        self.maintBlock.clear()
        # Remove the slice so that all blocks are added.
    ...
    def update_all(self):
        ...
        if self.red_track_controller:
            self.ctc.red_switch_states = self.red_track_controller.switch_states.copy()
            self.ctc.red_light_states = self.red_track_controller.light_states.copy()
            self.ctc.red_crossing_states = self.red_track_controller.crossing_states.copy()
        self.ctc_office.update()
        self.update_train_table()
        self.update_block_occupancy_table()
        self.update_track_states()
        self.update_red_track_states()
    ...
    def update_train_table(self):
        ...
        for row, train in enumerate(trains):
            ...
            items = [
                ...
                QTableWidgetItem(self.get_station_name(next_stop, train.line_name) if isinstance(next_stop, int) else str(next_stop))
            ]
            for col, item in enumerate(items):
                item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                self.tableTrainMonitor.setItem(row, col, item)

    def get_station_name(self, block: int, line: str) -> str:
        # Decide which dictionary to use
        if line.lower().strip() == "red line":
            return STATION_BLOCKS_RED["BLOCK_TO_STATION"].get(block, f"Block {block}")
        else:
            return STATION_BLOCKS_GREEN["BLOCK_TO_STATION"].get(block, f"Block {block}")
    ...
    def update_red_track_states(self):
        mapping = [
            ("Crossing 1", self.ctc.red_crossing_states[10]),  # Block 11
            ("Crossing 2", self.ctc.red_crossing_states[46]),  # Block 47
            ("Switch 1", self.ctc.red_switch_states[7]),  # Block 8
            ("Light 1", self.ctc.red_light_states[0]),
            ("Switch 2", self.ctc.red_switch_states[14]),  # Block 15
            ("Light 2", self.ctc.red_light_states[1]),
            ("Switch 3", self.ctc.red_switch_states[50]),  # Block 51
            ("Switch 4", self.ctc.red_switch_states[42]),  # Block 43
            ("Light 3", self.ctc.red_light_states[2]),
            ("Switch 5", self.ctc.red_switch_states[36]),  # Block 37
            ("Switch 6", self.ctc.red_switch_states[31]),  # Block 32
            ("Light 4", self.ctc.red_light_states[3]),
            ("Switch 7", self.ctc.red_switch_states[25]),  # Block 26
        ]

        self.TrackStates_2.setRowCount(len(mapping))
    ...
    def manual_stop_selection(self):
        ...
        if not ok:
            return
        stops = []
        # Use the proper block list based on the selected line.
        if line == "Green Line":
        ...
        # Add an option to finish the selection.
        possible_stops.insert(0, "Done")
        while True:
            stop_item, ok = QInputDialog.getItem(self, "Select Stop",
                                                 "Select a stop (choose 'Done' when finished):",
        ...
        # Schedule the train manually using the selected line and stops.
        self.ctc_office.schedule_manual_train(line, train_id, stops)
        self.update_all()
        QMessageBox.information(self, "Success",
                                f"Manually scheduled Train {train_id} on {line} with stops: {stops}")
