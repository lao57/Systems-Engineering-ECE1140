...
if __name__ == "__main__":
    ...
    def sync_backend_with_controllers(green_ctrl, red_ctrl, track_model):
        # Sync Green Line (Blocks 1–150)
        for block_num in range(1, 151):
            switch_state = green_ctrl.switch_states[block_num % len(green_ctrl.switch_states)]
            light_state = green_ctrl.light_states[block_num % len(green_ctrl.light_states)]
            crossing_state = green_ctrl.crossing_states[block_num % len(green_ctrl.crossing_states)]
            authority = green_ctrl.get_block_authority()[block_num - 1]
        ...
        # Sync Red Line (Blocks 1–76)
        for block_num in range(1, 77):
            switch_state = red_ctrl.switch_states[block_num % len(red_ctrl.switch_states)]
            light_state = red_ctrl.light_states[block_num % len(red_ctrl.light_states)]
            crossing_state = red_ctrl.crossing_states[block_num % len(red_ctrl.crossing_states)]
            authority = red_ctrl.get_block_authority()[block_num - 1]
        ...
        print("[Main] Backend synced with both Track Controllers")
