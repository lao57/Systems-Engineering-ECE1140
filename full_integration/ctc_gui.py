...
from ctc_office.ctc_office_ui import Ui_MainWindow
...
class CTCGUI(QMainWindow, Ui_MainWindow):
    ...
    def setup_connections(self):
        self.scheduleTrainLine.currentTextChanged.connect(self.update_combo_colors)
        self.maintLine.currentTextChanged.connect(self.update_combo_colors)
    ...
    def update_combo_colors(self):
        # Update scheduleTrainLine color
        self.scheduleTrainLine.setStyleSheet(
            "QComboBox { background-color: green; color: white; }"
            if self.scheduleTrainLine.currentText() == "Green Line"
            else "QComboBox { background-color: red; color: white; }"
        )

        # Update maintLine color
        self.maintLine.setStyleSheet(
            "QComboBox { background-color: green; color: white; }"
            if self.maintLine.currentText() == "Green Line"
            else "QComboBox { background-color: red; color: white; }"
        )
    ...
    def load_schedule(self):
        ...
        if path:
            try:
                ...
                # Get the selected train line from the dropdown
                selected_line = self.scheduleTrainLine.currentText().strip()

                # Only load schedule for selected line
